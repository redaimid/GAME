...
module.exports = async function (db, vk, context, limits) {
    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'down7up' && context.messagePayload && context.messagePayload.command == 'down7up_bet_down') {
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId].down && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'down7up') {
                currentStavka += Math.floor(db.gamesData[i].convGame.bets[context.senderId].down)
                console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].down)
            }
        }
        currentStavka += Number(_coin)
        if (Number(currentStavka > db.botSettings.gamesSettings.Down7Up.maxBets.down)) {
            return context.send({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, общая сумма ставок на выпадение числа меньше 7 не должна превышать ${util.number_format(db.botSettings.gamesSettings.Down7Up.maxBets.down)}`
            })
        }
        ...
        if (!db.gamesData[context.peerId].convGame.bets[context.senderId].down) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].down = 0
        }

        db.gamesData[context.peerId].convGame.bets[context.senderId].down += Math.floor(_coin)
        ...
        if (Number(db.playersData[context.senderId].balance) >= Number(_coin)) {
            ...
            context.send(`${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, успешная ставка ${util.number_format(_coin)} коинов на выпадение числа меньше 7`)
