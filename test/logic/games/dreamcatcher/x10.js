...
module.exports = async function(db, vk, context, limits) {
    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'dreamcatcher' && context.messagePayload && context.messagePayload.command == 'dreamcatcher_bet_x10') {
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId].x10 && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'dreamcatcher') {
                currentStavka += Math.floor(db.gamesData[i].convGame.bets[context.senderId].x10)
                console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].x10)
            }
        }
        currentStavka += Number(_coin)
        if (Number(currentStavka > db.botSettings.gamesSettings.dreamCatcher.maxBets.x10)) {
            return context.send({
        ...
        if (!db.gamesData[context.peerId].convGame.bets[context.senderId].x10) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].x10 = 0
        }

        db.gamesData[context.peerId].convGame.bets[context.senderId].x10 += Math.floor(_coin)
       ...
       if (Number(db.playersData[context.senderId].balance) >= Number(_coin)) {
        ...
        context.send(`${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, успешная ставка ${util.number_format(_coin)} коинов на коэффициент x10`)
