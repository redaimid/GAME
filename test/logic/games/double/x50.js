...
module.exports = async function (db, vk, context, limits) {
    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'double' && context.messagePayload && context.messagePayload.command == 'double_bet_x50') {
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId].x50 && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'double') {
                currentStavka += Math.floor(db.gamesData[i].convGame.bets[context.senderId].x50)
                console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].x50)
            }
        }
        currentStavka += Number(_coin)
        if (Number(currentStavka > db.botSettings.gamesSettings.Double.maxBets.x50)) {
            return context.send({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, общая сумма ставок на коэффициент x50 не должна превышать ${util.number_format(db.botSettings.gamesSettings.Double.maxBets.x50)}`
            })
        }
        ...
        if (!db.gamesData[context.peerId].convGame.bets[context.senderId].x50) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].x50 = 0
        }

        db.gamesData[context.peerId].convGame.bets[context.senderId].x50 += Math.floor(_coin)
        ...
        if (scale >= _coin) {
                 ...
                 if (Number(db.playersData[context.senderId].balance) >= Number(_coin)) {
                    ...
                    context.send(`${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, успешная ставка ${util.number_format(_coin)} коинов на коэффициент x50`)
