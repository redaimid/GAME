...
module.exports = async function (db, vk, context, limits) {
    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'dice' && context.messagePayload && context.messagePayload.command == 'dice_bet_four') {
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId].four && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'dice') {
                currentStavka += Math.floor(db.gamesData[i].convGame.bets[context.senderId].four)
                console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].four)
            }
        }
        ...
        if (!db.gamesData[context.peerId].convGame.bets[context.senderId].four) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].four = 0
        }

        db.gamesData[context.peerId].convGame.bets[context.senderId].four += Math.floor(_coin)
        ...
        if (scale >= _coin) {
                 ...
                 if (Number(db.playersData[context.senderId].balance) >= Number(_coin)) {
                    ...
                    context.send(`${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, успешная ставка ${util.number_format(_coin)} коинов на число 4`)
