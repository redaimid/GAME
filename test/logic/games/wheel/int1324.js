...
module.exports = async function (db, vk, context, limits) {
    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'wheel' && context.messagePayload && context.messagePayload.command == 'wheel_bet_int1324') {
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId].int1324 && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'wheel') {
                currentStavka += Math.floor(db.gamesData[i].convGame.bets[context.senderId].int1324)
                console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].int1324)
            }
        }
        ...
        if (!db.gamesData[context.peerId].convGame.bets[context.senderId].int1324) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].int1324 = 0
        }
        db.gamesData[context.peerId].convGame.bets[context.senderId].int1324 += Math.floor(_coin)
 ...
 let amountToTake = Number(_coin) - Number(db.playersData[context.senderId].balance)
        ...
        if (Number(db.playersData[context.senderId].balance) >= Number(_coin)) {
            ...
            context.send(`${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, успешная ставка ${util.number_format(_coin)} коинов на промежуток 13-24`)
