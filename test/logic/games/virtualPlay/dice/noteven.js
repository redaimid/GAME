...
module.exports = async function (db, vk, context, limits) {
    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'dice' && context.messagePayload && context.messagePayload.command == 'dice_bet_noteven') {
        ...
        if (scale <= 0 || db.playersData[context.senderId].userData.globalSettings.allowInlineButtons == false) {
            _coin = await context.question({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, введи сумму ставки на нечетное:`
            })
        }
        if (scale > 0 && db.playersData[context.senderId].userData.globalSettings.allowInlineButtons == true) {
            _coin = await context.question({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, введи сумму ставки на нечетное ИЛИ нажми на кнопку:`,
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId].vnoteven && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'dice') {
                currentStavka += Math.floor(db.gamesData[i].convGame.bets[context.senderId].vnoteven)
                console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].vnoteven)
            }
        }
        currentStavka += Number(_coin)
        if (Number(currentStavka > db.botSettings.gamesSettings.Dice.maxBets.evenNoteven)) {
            return context.send({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, общая сумма ставок на нечетное не должна превышать ${util.number_format(db.botSettings.gamesSettings.Dice.maxBets.evenNoteven)}`
            })
        }
        ...
        if (!db.gamesData[context.peerId].convGame.bets[context.senderId].vnoteven) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].vnoteven = 0
        }

        db.gamesData[context.peerId].convGame.bets[context.senderId].vnoteven += Math.floor(_coin)
        ...
        if (scale >= _coin) {
                    ...
                    context.send(`${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, успешная ставка ${util.number_format(_coin)} ${config.botVirtualCurrency} на нечетное`)
                    return admin.getDiceAdminMenu(db, vk, context, limits, _coin, 'нечетное')
