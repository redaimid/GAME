...
module.exports = async function (db, vk, context, limits) {
    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'dice' && context.messagePayload && context.messagePayload.command == 'dice_bet_six') {
        ...
        if (scale <= 0 || db.playersData[context.senderId].userData.globalSettings.allowInlineButtons == false) {
            _coin = await context.question({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, введи сумму ставки на число 6:`
            })
        }
        if (scale > 0 && db.playersData[context.senderId].userData.globalSettings.allowInlineButtons == true) {
            _coin = await context.question({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, введи сумму ставки на число 6 ИЛИ нажми на кнопку:`,
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId].vsix && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'dice') {
                currentStavka += Math.floor(db.gamesData[i].convGame.bets[context.senderId].vsix)
                console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].vsix)
            }
        }
        currentStavka += Number(_coin)
        if (Number(currentStavka > db.botSettings.gamesSettings.Dice.maxBets.numbers)) {
            return context.send({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, общая сумма ставок на число 6 должна превышать ${util.number_format(db.botSettings.gamesSettings.Dice.maxBets.numbers)}`
            })
        }
        ...
        if (!db.gamesData[context.peerId].convGame.bets[context.senderId].vsix) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].vsix = 0
        }

        db.gamesData[context.peerId].convGame.bets[context.senderId].vsix += Math.floor(_coin)
        ...
        if (scale >= _coin) {
                    ...
                    context.send(`${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, успешная ставка ${util.number_format(_coin)} ${config.botVirtualCurrency} на число 6`)
                    return admin.getDiceAdminMenu(db, vk, context, limits, _coin, 'число 6')
