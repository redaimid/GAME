...
module.exports = async function (db, vk, context, limits) {
    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'wheel' && context.messagePayload && context.messagePayload.command == 'wheel_bet_int2536') {
        ...
        if (scale <= 0 || db.playersData[context.senderId].userData.globalSettings.allowInlineButtons == false) {
            _coin = await context.question({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, введи сумму ставки на промежуток 25-36:`
            })
        }
        if (scale > 0 && db.playersData[context.senderId].userData.globalSettings.allowInlineButtons == true) {
            _coin = await context.question({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, введи сумму ставки на промежуток 25-36 ИЛИ нажми на кнопку:`,
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId].vint2536 && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'wheel') {
                currentStavka += Math.floor(db.gamesData[i].convGame.bets[context.senderId].vint2536)
                console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].vint2536)
            }
        }
        ...
        if (!db.gamesData[context.peerId].convGame.bets[context.senderId].vint2536) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].vint2536 = 0
        }
        db.gamesData[context.peerId].convGame.bets[context.senderId].vint2536 += Math.floor(_coin)
 ...
 let amountToTake = Number(_coin) - Number(db.playersData[context.senderId].vbalance)
        ...
        // Снятие основного баланса
            ...
            context.send(`${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, успешная ставка ${util.number_format(_coin)} ${config.botVirtualCurrency} на промежуток 25-36`)
            return admin.getWheelAdminMenu(db, vk, context, limits, _coin, 'промежуток 25-36')
