    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'wheel' && context.messagePayload && context.messagePayload.command == 'wheel_bet_numbers') {
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'wheel') {
                if(db.gamesData[i].convGame.bets[context.senderId].vnumbers && db.gamesData[i].convGame.bets[context.senderId].vnumbers[number.text]) {
                    currentStavka += Math.floor(db.gamesData[i].convGame.bets[context.senderId].vnumbers[number.text].amount)
                    console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].vnumbers[number.text].amount)
                }
            }
        }
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'wheel') {
                if(db.gamesData[i].convGame.bets[context.senderId].vnumbers) { 
                    for(d in db.gamesData[i].convGame.bets[context.senderId].vnumbers) {
                        currentSum += Math.floor(db.gamesData[i].convGame.bets[context.senderId].vnumbers[d].amount)
                        console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].vnumbers[d].amount)
                    }
                }
            }
        }
        ...
        if (Number(currentSum > db.botSettings.gamesSettings.Wheel.maxBets.numberSum)) {
            return context.send({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, общая сумма ставок на числа не должна превышать ${util.number_format(db.botSettings.gamesSettings.Wheel.maxBets.numberSum)} ${config.botVirtualCurrency}`
            })
        }
        ...
        if (!db.gamesData[context.peerId].convGame.bets[context.senderId].vnumbers) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].vnumbers = {}
        }

        if(!db.gamesData[context.peerId].convGame.bets[context.senderId].vnumbers[number.text]) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].vnumbers[number.text] = {
                amount: 0,
                number: number.text
            }
        }
        db.gamesData[context.peerId].convGame.bets[context.senderId].vnumbers[number.text].amount += Math.floor(_coin)
...
scale = Math.floor(db.playersData[context.senderId].vbalance)
        ...
        // Снятие основного баланса
            ...
            context.send(`${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, успешная ставка ${util.number_format(_coin)} ${config.botVirtualCurrency} на выпадение числа ${number.text}`)
