...
module.exports = async function (db, vk, context, limits) {
    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'wheel' && context.messagePayload && context.messagePayload.command == 'wheel_bet_noteven') {
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId].vnoteven && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'wheel') {
 ...
 let amountToTake = Number(_coin) - Number(db.playersData[context.senderId].vbalance)
        ...
        // Снятие основного баланса
            ...
            return admin.getWheelAdminMenu(db, vk, context, limits, _coin, 'нечетное')
