...
module.exports = async function (db, vk, context, limits) {
    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'double' && context.messagePayload && context.messagePayload.command == 'double_bet_x5') {
        ...
        if (scale <= 0 || db.playersData[context.senderId].userData.globalSettings.allowInlineButtons == false) {
            _coin = await context.question({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, введи сумму ставки на коэффициент x5:`
            })
        }
        if (scale > 0 && db.playersData[context.senderId].userData.globalSettings.allowInlineButtons == true) {
            _coin = await context.question({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, введи сумму ставки на коэффициент x5 ИЛИ нажми на кнопку:`,
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId].vx5 && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'double') {
                currentStavka += Math.floor(db.gamesData[i].convGame.bets[context.senderId].vx5)
                console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].vx5)
            }
        }
        currentStavka += Number(_coin)
        if (Number(currentStavka > db.botSettings.gamesSettings.Double.maxBets.vx5)) {
            return context.send({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, общая сумма ставок на коэффициент x5 не должна превышать ${util.number_format(db.botSettings.gamesSettings.Double.maxBets.vx5)}`
            })
        }
        ...
        if (!db.gamesData[context.peerId].convGame.bets[context.senderId].vx5) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].vx5 = 0
        }

        db.gamesData[context.peerId].convGame.bets[context.senderId].vx5 += Math.floor(_coin)
        ...
        // Снятие основного баланса
                ...
                context.send(`${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, успешная ставка ${util.number_format(_coin)} ${config.botVirtualCurrency} на коэффициент x5`)
                return admin.getDoubleAdminMenu(db, vk, context, limits, _coin, 'x5')
