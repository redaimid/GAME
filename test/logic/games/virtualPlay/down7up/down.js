...
module.exports = async function (db, vk, context, limits) {
    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'down7up' && context.messagePayload && context.messagePayload.command == 'down7up_bet_down') {
        if(db.playersData[context.senderId].userData.globalSettings.isVirtualPlay == false) return

        if (db.gamesData[context.peerId].convGame.bets[context.senderId] && db.gamesData[context.peerId].convGame.bets[context.senderId].vseven > 0 || db.gamesData[context.peerId].convGame.bets[context.senderId] && db.gamesData[context.peerId].convGame.bets[context.senderId].vup > 0 || db.gamesData[context.peerId].convGame.bets[context.senderId] && db.gamesData[context.peerId].convGame.bets[context.senderId].seven > 0 || db.gamesData[context.peerId].convGame.bets[context.senderId] && db.gamesData[context.peerId].convGame.bets[context.senderId].up > 0) {
            return context.send({
        ...
        if (scale <= 0 || db.playersData[context.senderId].userData.globalSettings.allowInlineButtons == false) {
            _coin = await context.question({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, введи сумму ставки на выпадение числа меньше 7:`
            })
        }
        if (scale > 0 && db.playersData[context.senderId].userData.globalSettings.allowInlineButtons == true) {
            _coin = await context.question({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, введи сумму ставки на выпадение числа меньше 7 ИЛИ нажми на кнопку:`,
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId].vdown && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'down7up') {
                currentStavka += Math.floor(db.gamesData[i].convGame.bets[context.senderId].vdown)
                console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].vdown)
            }
        }
        currentStavka += Number(_coin)
        if (Number(currentStavka > db.botSettings.gamesSettings.Down7Up.maxBets.vdown)) {
            return context.send({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, общая сумма ставок на выпадение числа меньше 7 не должна превышать ${util.number_format(db.botSettings.gamesSettings.Down7Up.maxBets.vdown)}`
            })
        }
        ...
        if (!db.gamesData[context.peerId].convGame.bets[context.senderId].vdown) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].vdown = 0
        }

        db.gamesData[context.peerId].convGame.bets[context.senderId].vdown += Math.floor(_coin)
        ...
        // Снятие основного баланса
            ...
            context.send(`${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, успешная ставка ${util.number_format(_coin)} ${config.botVirtualCurrency} на выпадение числа меньше 7`)
            return admin.getDown7UpAdminMenu(db, vk, context, limits, _coin, 'число меньше 7')
