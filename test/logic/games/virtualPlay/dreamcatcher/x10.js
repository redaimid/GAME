...
module.exports = async function(db, vk, context, limits) {
    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'dreamcatcher' && context.messagePayload && context.messagePayload.command == 'dreamcatcher_bet_x10') {
        ...
        if (scale <= 0 || db.playersData[context.senderId].userData.globalSettings.allowInlineButtons == false) {
            _coin = await context.question({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, введи сумму ставки на коэффициент x10:`
            })
        }
        if (scale > 0 && db.playersData[context.senderId].userData.globalSettings.allowInlineButtons == true) {
            _coin = await context.question({
                message: `${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, введи сумму ставки на коэффициент x10 ИЛИ нажми на кнопку:`,
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId].vx10 && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'dreamcatcher') {
                currentStavka += Math.floor(db.gamesData[i].convGame.bets[context.senderId].vx10)
                console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].vx10)
            }
        }
        currentStavka += Number(_coin)
        if (Number(currentStavka > db.botSettings.gamesSettings.dreamCatcher.maxBets.vx10)) {
            return context.send({
        ...
        if (!db.gamesData[context.peerId].convGame.bets[context.senderId].vx10) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].vx10 = 0
        }

        db.gamesData[context.peerId].convGame.bets[context.senderId].vx10 += Math.floor(_coin)
        ...
        if (scale >= _coin) {
            ...
            context.send(`${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, успешная ставка ${util.number_format(_coin)} ${config.botVirtualCurrency} на коэффициент x10`)
            return admin.getDreamCatcherAdminMenu(db, vk, context, limits, _coin, 'x10')
        }
    }
